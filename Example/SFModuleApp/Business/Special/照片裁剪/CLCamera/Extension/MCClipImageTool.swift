...
public extension UIAlertController {
    static func showActionSheet(on vc: UIViewController, items: [MCClipImageScaleType], confirm: ((Int, CGFloat) -> Void)?, cancel: ((UIAlertAction) -> Void)? = nil) {
        let alter = UIAlertController(title: "重设裁切框的比例", message: nil, preferredStyle: .actionSheet)

        let cancle = UIAlertAction(title: "取消", style: .cancel, handler: cancel)

        var index = 0
        for item in items {
            let i = index
            let confirm = UIAlertAction(title: item.describe(), style: UIAlertAction.Style.default) { b in
...
extension UIImage {
    ...
    func rotationImage(orientation: UIImage.Orientation) -> UIImage {
        ...
        case .left:
            rotate = Double.pi / 2
            rect = CGRect(x: 0, y: 0, width: imageHeight, height: imageWidth)
            translateX = 0
    ...
    func judgeRelationTypeWithCropSize(_ cropSize: CGSize) -> Int {
        ...
        let image_W = size.width
        let image_H = size.height
    ...
    public func clipCircularImage() -> UIImage {
        ...
        let context = UIGraphicsGetCurrentContext()
        context!.beginPath()
        context?.addArc(center: CGPoint(x: arcCenterX, y: arcCenterY), radius: radius, startAngle: 0, endAngle: CGFloat.pi * 2, clockwise: false)
        context?.clip()
...
@objc public extension Bundle {
     ...
     * @return UIImage
     */
    static func MCLoadImageFromBundleName(_ bundleName: String, podName: String, imageName: String) -> UIImage? {
