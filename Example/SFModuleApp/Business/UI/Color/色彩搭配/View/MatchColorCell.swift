...
class MatchColorCell: BaseCollectionViewCell {
    
    var defLayout: FlowLayoutType = .single
    
    private var km: MatchColorModel?
    ...
    override func loadData(_ model: BaseModel, indexPath: IndexPath) {
        if let m = model as? MatchColorModel {
            self.km = m
            ...
            infoLabel.text = self.defLayout == .single ? (m.color1 + " -> " + m.color2 + " -> " + m.color3 + " -> " + m.color4) : "color-\(indexPath.item)"
        }
    }
