...
/*****  默认值 *****/
// 中间指示器顶部闭合三角形默认高度
#define kTriangle_Height_Default 4
// 中间指示器距离顶部默认距离
#define kIndicator_Pad_Top_Default 4
// 中间指示器距离底部默认距离
#define kIndicator_Pad_Bottom_Default 4
// 中间指示器默认颜色
#define kIndicator_Color_Default [UIColor redColor]
// 标尺刻度线两个刻度线的间距
...
// 标尺刻度线大刻度线的颜色
#define kGraduated_Main_Line_Color_Default [UIColor colorWithRed:0.6 green:0.6 blue:0.6 alpha:1.0];
// 标尺刻度线小刻度线的颜色
#define kGraduated_Secondary_Line_Color_Default [UIColor colorWithRed:0.8163 green:0.8162 blue:0.8162 alpha:1.0];
// 默认高度
...
- (VNBRulerScrollView *)configureRulerScrollView {
    ...
    rulerScrollView.mainLineColor             = self.mainLineColor.CGColor;
    rulerScrollView.mainLineHeight            = self.mainLineHeight;
    rulerScrollView.secondaryLineColor        = self.secondaryLineColor.CGColor;
...
//    rulerScrollView.accuracy                  = _accuracy;
    ...
    rulerScrollView.startTime                 = self.start;
...
- (void)drawRacAndLine {
    ...
    shapeLayerLine.strokeColor = _indicatorColor.CGColor; // 指示器颜色
    shapeLayerLine.fillColor = _indicatorColor.CGColor; //三角颜色
...
- (void)commonInitProperty{
    ...
    _start                     = 8;//默认从8点开始,根据开始时间和大刻度个数计算卡尺的开始时间和结束时间，保证返回值的正确性
...
- (void)seekToRulerValue:(NSString *)rulerValue{
    ...
    NSInteger hour = [dateArray[0] integerValue] - _start;
...
- (NSString *)getTimeBySmallUnitOffest:(CGFloat)unitOffest{
    ...
    NSInteger hour = unitOffest / 60 + _start;
...
- (void)drawRulerGraduatedLine{
    ...
    for (int i = 0; i <= _numberOfGraduatedLines; i++) {
        if (i % _numberOfSmallUnit == 0) {
            ...
            rule.text = [NSString stringWithFormat:@"%02ld:00",i/_numberOfSmallUnit+_startTime];
    ...
    if (self.rulerValue) {
        ...
        NSInteger hour = [array[0] integerValue] - _startTime;
